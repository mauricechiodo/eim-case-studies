...
---
 ...
 $$ \begin{aligned}
 \text{(i)} ~ & ~ X(0) = 0 ~; \\\\
 \text{(ii)} ~ & ~ \lbrace X(t), ~ t \geqslant 0 \rbrace \text{ has stationary and independent increments, in that the distribution of $X(s+t) - X(t)$ does not depend on $t$, \linebreak
