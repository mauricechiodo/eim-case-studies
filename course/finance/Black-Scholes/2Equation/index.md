...
The underlying assumption to Black and Scholes' work is that the underlying stock price $S$ follos a geometric Brownian motion with drift $\mu$ and variance parameter $\sigma^2$:
 ...
 (i) $X(0) = 0$;  
 (ii) $\lbrace X(t), t \geqlsant 0 \rbrace$ has stationary and independent increments, in that the distribution of $X(t+s) - X(t)$ does not depend on $t$, and for all $t_1 < t_2 < ... < t_n$, $X(t_n) - X(t_{n-1})$, $X(t_{n-1}) - X(t_{n-2})$, ..., $X(t_2) - X(t_1)$, $X(t_1)$ are independent;  
